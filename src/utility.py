...
def data_prep(
    ...
    df = pd.merge(
        transaction[["t_dat", "customer_id", "article_id", "price"]],
...
def individual_item_detailed(
    df: pd.DataFrame, products: pd.DataFrame, product_name: str, n: int
):
    ...
    least_frequent = {}
    cnt2 = df.loc[
        (df.order_instance.isin(orders)) & (df["prod_name"] != product_name), "prod_name"
    ].value_counts()
    ...
    often_rare_purchased = pd.DataFrame(
        {
            "prod_name": list(most_frequent.keys()),
    ...
    often_rare_purchased = pd.merge(
        often_rare_purchased,
        products[
    ...
    for i in range(1, n + 1):
        often_rare_purchased[f"{i}_freq_prod_lift"] = (
            temp[most_purchased[i - 1]] / n_order
        ) / (support[most_frequent[product_name][i - 1]] * support[product_name])
    frequent_groups = [
        list(
            set(
                products[
                    products["prod_name"] == often_rare_purchased.often_purchased[0][i]
                ]["product_group_name"]
            )
        )[0]
        for i in range(n)
