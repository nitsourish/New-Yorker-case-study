...
def prepare_all_products_report(df:pd.DataFrame,product_hierarchy:str,num_prod:int,all:bool=True,prods:list=[]):
...
if __name__ == '__main__':
    ...
    df = data_prep(transaction = train,products=products,min_cnt = 0,penetration=args.penetration)
    print(f'Data prepared {df.shape}')

    often_rare_purchased,support = prepare_all_products_report(df,product_hierarchy=args.product_hierarchy,num_prod=args.num_prod,all=args.all,prods=args.prods)
    print(often_rare_purchased.head())
    often_rare_purchased.to_csv(os.path.join(report_path,'prod_name_wise_report_adhoc.csv'),index=False)
